...
static u64 __sched_period(unsigned long nr_running)
{
	...
	if (unlikely(nr_running > sched_nr_latency)) {
		...
		if (nr_running > 1)
			period = div_u64(period * nr_running, sched_nr_latency);
		period = max_t(u64, period, 1000000ULL);
	}
	return period;
}
...
static u64 sched_slice(struct cfs_rq *cfs_rq, struct sched_entity *se)
{
	...
	return max_t(u64, slice, 1000000ULL);
}
...
#ifdef CONFIG_SCHED_BORE
	/*
	 * BORE: scale_slice() inflates ideal_runtime via burst_score weight.
	 * Use the raw unscaled period for the preemption threshold so that
	 * CPU-bound tasks cannot use their own vruntime inflation to avoid
	 * being preempted at the tick.
	 */
	ideal_runtime = max_t(u64,
		__calc_delta(__sched_period(cfs_rq->nr_running),
			     curr->load.weight, &cfs_rq->load),
		1000000ULL);
#else
	...
	for (; pd; pd = pd->next) {
		...
		for_each_cpu_and(cpu, pd_mask, cpu_online_mask) {
			...
			sum_util += schedutil_cpu_util(cpu, util_cfs_energy, cpu_cap,
						       ENERGY_UTIL, NULL);
			/* OSK: clamp sum to prevent overloaded CPU inflating domain energy */
			sum_util = min(sum_util,
				(unsigned long)cpumask_weight(pd_mask) * cpu_cap);
